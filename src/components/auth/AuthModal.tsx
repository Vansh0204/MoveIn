...
export default function AuthModal() {
  const { isAuthModalOpen, closeAuthModal, login, showToast } = useAuth();
  ...
  // Early return if not open so we don't render content unnecessarily
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    showToast(`Welcome to MoveIn, ${userName}! 👋`);
    closeAuthModal();
  };
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      >
        <button onClick={closeAuthModal} aria-label="Close modal" className="absolute top-4 right-4 text-brand-ink/40 hover:text-brand-ink hover:bg-gray-100 p-1 rounded-full z-10 transition-colors">
          <X size={20} />
        </button>
        ...
        <div className="flex border-b border-black/5 relative shrink-0">
          {(["signin", "signup"] as const).map(t => (
            ...
            >
              ...
              {tab === t && (
                <motion.div layoutId="tabIndicator" className="absolute bottom-0 h-0.5 bg-brand-gold w-1/2" style={{ left: t === "signin" ? '0%' : '50%' }} />
              )}
            </button>
          ))}
        </div>
        ...
        <div className="p-6 overflow-y-auto">
          ...
          <button className="w-full flex items-center justify-center space-x-2 bg-white border border-brand-gold/50 rounded-full py-3 shadow-sm hover:bg-brand-sand/30 transition-colors mb-6 group">
            ...
            <span className="font-bold text-[14px] text-brand-ink">Continue with Google</span>
          </button>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <input 
                type="email" placeholder="Email address" value={email} onChange={e => setEmail(e.target.value)} onBlur={() => handleBlur("email")}
            ...
            {tab === "signup" && (
              <div>
                <input 
                  type={showPassword ? "text" : "password"} placeholder="Confirm password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)}
              ...
              <div className="pt-2">
                ...
                <div className="flex space-x-2">
                  {['Boys PG', 'Girls PG', 'Co-Ed'].map(g => (
                    <button type="button" key={g} onClick={() => setGender(g)} className={`px-4 py-2 rounded-full text-[12px] font-bold transition-colors ${gender === g ? 'bg-brand-ink text-white shadow-md' : 'bg-gray-100 text-brand-ink/60 hover:bg-gray-200'}`}>
                      {g}
                    </button>
                  ))}
                </div>
            ...
            {tab === "signup" && (
              <div className="flex items-start mt-4 pt-2">
                <input type="checkbox" id="terms" checked={agreeTerms} onChange={e => setAgreeTerms(e.target.checked)} className="mt-0.5 mr-3 w-4 h-4 rounded border-gray-300 text-brand-gold focus:ring-brand-gold cursor-pointer" />
            ...
            <button type="submit" className="w-full bg-brand-gold text-brand-ink font-bold text-[15px] py-4 rounded-xl hover:bg-brand-ink hover:text-white transition-all shadow-md mt-6 active:scale-[0.98]">
          ...
          <div className="text-center mt-6">
            {tab === "signin" ? (
              <p className="text-[14px] text-brand-ink/60 font-medium">New to MoveIn? <button onClick={() => { setTab("signup"); setErrors({}); }} className="text-brand-gold font-bold hover:text-brand-ink transition-colors">Create account</button></p>
            ) : (
              <p className="text-[14px] text-brand-ink/60 font-medium">Already have an account? <button onClick={() => { setTab("signin"); setErrors({}); }} className="text-brand-gold font-bold hover:text-brand-ink transition-colors">Sign in</button></p>
            )}
          </div>
        </div>
      </motion.div>
